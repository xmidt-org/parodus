...
static void *serviceAliveTask()
{
	...
	while(1)
	{
		cimplog_info("PARODUS", "serviceAliveTask: numOfClients registered is %d\n", numOfClients);
		if(numOfClients > 0)
		{
			...
			do
			{
				...
				if(byte == size)
				{
					cimplog_debug("PARODUS", "Keep alive msg sent to client\n");
					cimplog_info("PARODUS", "temp->service_name: %s is alive\n",temp->service_name);
				}
				else
				{
					cimplog_debug("PARODUS", "Failed to send keep alive msg to client\n");
					cimplog_info("PARODUS", "service %s is dead\n", temp->service_name);
