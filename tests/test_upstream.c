...
size_t appendEncodedData( void **appendData, void *encodedBuffer, size_t encodedSize, void *metadataPack, size_t metadataSize )
{
    ...
    strncpy(data, "AAAAAAAAYYYYIGkYTUYFJH", strlen("AAAAAAAAYYYYIGkYTUYFJH")+1);
    *appendData = data;
    return (size_t)mock();
}
...
void err_processUpstreamMessageRegMsg()
{
    ...
    strncpy(head->next->service_name, "iot", strlen("iot")+1);
    strncpy(head->next->url, "tcp://10.0.0.1:6600", strlen("tcp://10.0.0.1:6600")+1);
    head->next->next = NULL;
