...
void handleUpstreamMessage(noPollConn *conn, void *msg, size_t len)
{
	...
	cimplog_info("PARODUS", "handleUpstreamMessage length %zu\n", len);
	if(nopoll_conn_is_ok(conn) && nopoll_conn_is_ready(conn))
	{
		...
		cimplog_debug("PARODUS", "Number of bytes written: %d\n", bytesWritten);
		if (bytesWritten != (int) len) 
		{
			cimplog_error("PARODUS", "Failed to send bytes %zu, bytes written were=%d (errno=%d, %s)..\n", len, bytesWritten, errno, strerror(errno));
		}
	}
	else
	{
		cimplog_error("PARODUS", "Failed to send msg upstream as connection is not OK\n");
	}
...
void listenerOnMessage(void * msg, size_t msgSize, int *numOfClients, reg_list_item_t **head )
{
	...
	cimplog_info("PARODUS", "Received msg from server:%s\n", recivedMsg);	
	if(recivedMsg!=NULL) 
	{
		...
		if(rv > 0)
		{
			...
			cimplog_info("PARODUS", "msgType received:%d\n", msgType);
			cimplog_debug("PARODUS", "numOfClients registered is %d\n", *numOfClients);
			...
			if((message->u.req.dest !=NULL))
			{
				...
				cimplog_info("PARODUS", "Received downstream dest as :%s\n", dest);
				temp = *head;
				//Checking for individual clients & Sending to each client
				
				do
				{
					cimplog_debug("PARODUS", "node is pointing to temp->service_name %s \n",temp->service_name);
					// Sending message to registered clients
					if( strcmp(dest, temp->service_name) == 0) 
					{
						cimplog_debug("PARODUS", "sending to nanomsg client %s\n", dest);     
						bytes = nn_send(temp->sock, recivedMsg, msgSize, 0);
						cimplog_info("PARODUS", "sent downstream message '%s' to reg_client '%s'\n",recivedMsg,temp->url);
						cimplog_debug("PARODUS", "downstream bytes sent:%d\n", bytes);
						destFlag =1;
						break;
				...
				if(destFlag ==0)
				{
					cimplog_error("PARODUS", "Unknown dest:%s\n", dest);
					...
					cimplog_info("PARODUS", "Payload Response: %s\n", str);
	  	...
	  	{
	  		cimplog_error("PARODUS",  "Failure in msgpack decoding for receivdMsg: rv is %d\n", rv );
	  	}
	  	
	  	cimplog_debug("PARODUS", "free for downstream decoded msg\n");
