...
int createObject( wrp_msg_t *reqMsg , wrp_msg_t **response)
{
	...
	if(reqMsg->u.crud.dest !=NULL)
	{
	   ...
	   ParodusInfo("destVal is %s\n", destVal);
		...
		if(tagObj !=NULL)
		{
			...
			if(testObj !=NULL)
			{
				...
				for( i = 0 ; j < jsontagitemSize ; j++ ) 
				{
					...
					{
						ParodusInfo( "testObj already exists. Update it\n" );
			...
			{
				ParodusInfo("testObj not exists, adding it\n");
		...
		{
			ParodusInfo("tagObj doesnot exists\n");
