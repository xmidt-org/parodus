...
//If new arguments are added, update argc accordingly.
    ...
    command[i++] = "--webpa-read-header=/tmp/token.sh";
    command[i++] = "--webpa-create-header=/tmp/token.sh";
    ...
    read_header_value(&parodusCfg);
    set_parodus_cfg(&parodusCfg);
    
    assert_string_equal(  get_parodus_cfg()->webpa_new_header,expectedToken);
