...
static noPollConn * nopoll_tls_common_conn (noPollCtx  * ctx,char * serverAddr,char *serverPort,char * extra_headers,int fallback);
		...
		if(get_global_conn() != NULL)
		{
			if(!nopoll_conn_is_ok(get_global_conn())) 
			{
				...
				if(FLAGS_IPV6_ONLY == (FLAGS_IPV6_IPV4 & fallback))
					fallback = FLAGS_IPV4_ONLY;
				else
					fallback = FLAGS_IPV6_ONLY;
...
static noPollConn * nopoll_tls_common_conn (noPollCtx  * ctx,char * serverAddr,char *serverPort,char * extra_headers,int fallback)
{
        ...
        if( FLAGS_IPV4_ONLY == (FLAGS_IPV4_ONLY & flags) ) {
			...
			if(!nopoll_conn_is_ok (connection) || FLAGS_IPV4_ONLY == (FLAGS_IPV6_IPV4 & fallback))
			{
